...
class AutoExecConfig:
    ...
    # Optional buffer for immediate_on_stage entries: adds a small $ buffer to desired_entry
    # when submitting the BUY limit order to improve fill probability.
    immediate_entry_buffer_enabled: bool = False
    immediate_entry_buffer_cents: float = 0.01
...
def autoexec_cfg_from_raw(raw: Any) -> AutoExecConfig:
    ...
    required = {
        ...
        "immediate_entry_buffer_enabled": False,
        "immediate_entry_buffer_cents": 0.01,
    }
    coercers = {
        ...
        "immediate_entry_buffer_enabled": lambda v: _coerce_bool(v, False),
        "immediate_entry_buffer_cents": lambda v: max(0.0, min(0.03, _coerce_float(v, 0.01))),
...
def try_send_entries(cfg: AutoExecConfig, allow_opening: bool, allow_midday: bool, allow_power: bool, fetch_last_price_fn) -> None:
    ...
    for symbol, lst in list(state.get("lifecycles", {}).items()):
        for idx, raw in enumerate(list(lst)):
            ...
            try:
                # Compute submitted entry limit. By default we submit desired_entry.
                submit_entry = float(lifecycle.desired_entry)
                # Optional buffer ONLY for immediate_on_stage mode (toggle in AutoExecConfig).
                if entry_mode == 'immediate_on_stage' and bool(getattr(cfg, 'immediate_entry_buffer_enabled', False)):
                    buf = float(getattr(cfg, 'immediate_entry_buffer_cents', 0.0) or 0.0)
                    # Clamp to 0.00–0.03 and require at least $0.01 to have effect.
                    buf = max(0.0, min(0.03, buf))
                    if buf >= 0.01:
                        submit_entry = submit_entry + buf
                submit_entry = float(_tick_round(submit_entry) or submit_entry)

                oid = client.place_equity_limit_order(
                    account_id_key=account_id_key,
                    symbol=symbol,
                    qty=lifecycle.qty,
                    limit_price=float(submit_entry),
                    action="BUY",
