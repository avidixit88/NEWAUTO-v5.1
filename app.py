...
with st.sidebar.expander("AUTO‑EXEC (E*TRADE) — LONG only", expanded=False):
    ...
    st.markdown('**Immediate entry buffer (optional)**')

    immediate_entry_buffer_enabled = st.checkbox(
        'Add buffer to immediate entry limit',
        value=False,
        disabled=(entry_mode != 'immediate_on_stage'),
        help=(
            'ONLY applies when Entry mode = Immediate on stage. '
            'Adds a small $ buffer (0.01–0.03) on top of desired_entry when submitting the BUY limit order. '
            'Example: desired_entry 1.11 with 0.01 buffer submits 1.12.'
        ),
    )

    immediate_entry_buffer_cents = st.slider(
        'Immediate entry buffer ($)',
        0.01, 0.03, 0.01, 0.01,
        disabled=(entry_mode != 'immediate_on_stage') or (not immediate_entry_buffer_enabled),
        help='Buffer added to desired_entry for the submitted BUY limit (immediate mode only).',
    )
    ...
    autoexec_cfg = AutoExecConfig(
        ...
        immediate_entry_buffer_enabled=bool(immediate_entry_buffer_enabled),
        immediate_entry_buffer_cents=float(immediate_entry_buffer_cents),
    )
